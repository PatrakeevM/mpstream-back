...
export const ms = (str: StringValue): number => {
  if (typeof str !== 'string' || str.length === 0 || str.length > 100) {
    throw new Error(
      'Value provided to ms() must be a string with length beyween 1 and 99.',
    );
  }

  const match =
    /^(?<value>-?(?:\d+)?\.?\d+) *(?<type>miliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)?$/i.exec(
      str,
    );
